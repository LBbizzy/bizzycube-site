  ...
  return (
    <>
      <section style={{ padding: "80px 24px 40px", maxWidth: 1000, margin: "0 auto", textAlign: "center" }}>
        <div style={{ fontSize: 11, color: "#B2D235", letterSpacing: ".14em", textTransform: "uppercase", fontWeight: 700, marginBottom: 14 }}>Pricing</div>
      ...
      <section style={{ padding: "30px 24px 60px", maxWidth: 1200, margin: "0 auto" }}>
        <div className="bz-grid bz-grid-3" style={{ gap: 18 }}>
          {[
            ...
            <div key={p.name} style={{ background: p.best ? "linear-gradient(180deg, rgba(83, 74, 183, 0.12), #0e1119)" : "#0e1119", border: p.best ? "2px solid #483A84" : "1px solid #1f2733", borderRadius: 14, padding: 28, position: "relative" }}>
              {p.best && <div style={{ position: "absolute", top: -12, left: 24, background: "#483A84", color: "#fff", padding: "4px 12px", borderRadius: 6, fontSize: 11, fontWeight: 700, letterSpacing: ".06em" }}>MOST POPULAR</div>}
              ...
              <Link href={p.cta} style={{ display: "block", textAlign: "center", background: p.best ? "#483A84" : "transparent", color: "#fff", border: p.best ? "none" : "1px solid #2c333f", padding: "12px 16px", borderRadius: 10, fontSize: 14, fontWeight: 700 }}>
                {p.ctaLabel}
              </Link>
            </div>
          ))}
            ...
            {DEPT_ORDER.map((slug, i) => {
              ...
              return (
                <Link key={slug} href={`/${slug}`} style={{ display: "grid", gridTemplateColumns: "50px 1fr 130px 30px", gap: 14, padding: "14px 18px", borderTop: i ? "1px solid #1f2733" : "none", fontSize: 14, alignItems: "center" }}>
                  ...
                  <div style={{ color: "#B2D235", textAlign: "right" }}>→</div>
                </Link>
              );
            })}
          </div>
        ...
        <div style={{ background: "#0e1119", border: "1px solid #1f2733", borderRadius: 14, overflow: "hidden" }}>
          {[
            ...
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1.6fr 0.4fr", padding: "16px 20px", borderTop: i ? "1px solid #1f2733" : "none", fontSize: 13.5, alignItems: "center", background: i === 0 ? "rgba(83, 74, 183, 0.08)" : "transparent" }}>
              <div style={{ color: i === 0 ? "#B2D235" : "#fff", fontWeight: i === 0 ? 700 : 500 }}>{r[0]}</div>
              <div style={{ color: "#cbd5e1" }}>{r[1]}</div>
              <div style={{ color: "#cbd5e1" }}>{r[2]}</div>
              <div style={{ color: r[3] === "✓" ? "#B2D235" : "#94a3b8", fontWeight: 700 }}>{r[3]}</div>
            </div>
          ))}
        </div>
      </section>
