  ...
  return (
    <>
      <section style={{ padding: "70px 24px 50px", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ fontSize: 11, color: "#B2D235", letterSpacing: ".14em", textTransform: "uppercase", fontWeight: 700, marginBottom: 12 }}>
          {d.category}
        </div>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 22, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <Link href={`/contact?cta=${d.slug}`} style={{ background: "#483A84", color: "#fff", padding: "14px 28px", borderRadius: 10, fontSize: 15, fontWeight: 700 }}>
                Get {d.name} →
              </Link>
        ...
        <div className="bz-grid bz-grid-3">
          {d.outcomes.map((o) => (
            <div key={o} style={{ background: "#0e1119", border: "1px solid #1f2733", borderRadius: 14, padding: 22 }}>
              <div style={{ color: "#B2D235", fontSize: 22, marginBottom: 6 }}>✓</div>
        ...
        <ul style={{ listStyle: "none", padding: 0, margin: 0, columnCount: 2, columnGap: 32 }}>
          {d.features.map(f => (
            <li key={f} style={{ fontSize: 14, color: "#cbd5e1", lineHeight: 1.6, padding: "8px 0", breakInside: "avoid" }}>
              <span style={{ color: "#B2D235", marginRight: 8 }}>▸</span>{f}
            </li>
          ))}
        </ul>
      </section>
      ...
      <section style={{ padding: "50px 24px", background: "#0a0d14", borderTop: "1px solid #1f2733", borderBottom: "1px solid #1f2733" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          {d.pricing.map(p => (
            <div key={p.tier} style={{ background: "#0e1119", border: "2px solid #483A84", borderRadius: 14, padding: 30 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 12 }}>
                <div>
                  <div style={{ fontSize: 11, color: "#B2D235", letterSpacing: ".14em", textTransform: "uppercase", fontWeight: 700 }}>Standalone</div>
              ...
              <Link href={`/contact?cta=${d.slug}`} style={{ display: "block", textAlign: "center", background: "#483A84", color: "#fff", padding: "14px 24px", borderRadius: 10, fontSize: 15, fontWeight: 700 }}>
                Get {d.name} →
              </Link>
              <div style={{ marginTop: 14, fontSize: 12, color: "#94a3b8", textAlign: "center" }}>
                Or bundle 3 departments and save 15%. <Link href="/pricing" style={{ color: "#B2D235" }}>See packages →</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "50px 24px", maxWidth: 800, margin: "0 auto" }}>
        <div style={{ background: "linear-gradient(135deg, #1e1b4b 0%, #0e1119 100%)", border: "1px solid #2c333f", borderRadius: 14, padding: 30 }}>
          <div style={{ fontSize: 11, color: "#B2D235", letterSpacing: ".14em", textTransform: "uppercase", fontWeight: 700, marginBottom: 8 }}>Proof</div>
          <p style={{ fontSize: 16, color: "#cbd5e1", lineHeight: 1.6, margin: 0 }}>{d.proof}</p>
          <Link href="/case-studies" style={{ color: "#B2D235", fontSize: 14, marginTop: 14, display: "inline-block" }}>{d.caseStudyHook} →</Link>
        </div>
      </section>

      <section style={{ padding: "50px 24px", background: "#0a0d14", borderTop: "1px solid #1f2733", borderBottom: "1px solid #1f2733" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ fontSize: 11, color: "#B2D235", letterSpacing: ".14em", textTransform: "uppercase", fontWeight: 700, marginBottom: 12, textAlign: "center" }}>The rest of the workforce</div>
            ...
            {d.alsoBuy.map(slug => {
              ...
              return (
                <Link key={slug} href={`/${slug}`} style={{ background: "#0e1119", border: "1px solid #1f2733", borderRadius: 12, padding: 18, display: "block" }}>
                  ...
                  <div style={{ marginTop: 10, fontSize: 12, color: "#B2D235" }}>Explore {o.name} →</div>
                </Link>
              );
            })}
          </div>
